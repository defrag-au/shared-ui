...
//!
//! ## Why Framework-Free Loading?
//!
//! The loading phase doesn't need reactive state management. It's just:
//! - DOM manipulation for the loading screen
//! - Fetch calls for data
//! - State machine transitions
//!
//! By running before Seed/Leptos/etc., we get:
//! - Instant loading feedback (no framework startup delay)
//! - Simpler framework code (no loading state to manage)
//! - Framework-agnostic (same loader for Seed, Leptos, macroquad, etc.)
//!
//! ## Usage
//!
//! ```ignore
//! use ui_loader::{LoadingOrchestrator, LoaderConfig};
//!
//! #[wasm_bindgen(start)]
//! pub async fn start() {
//!     let config = LoaderConfig::new().auth_required(true);
//!
//!     let result = LoadingOrchestrator::run(config, |auth, loader| async move {
//!         loader.set_message("Loading map...");
//!         let map = fetch_map().await?;
//!
//!         loader.set_message("Loading world state...");
//!         let world = fetch_world(&auth).await?;
//!
//!         Ok(MyData { map, world })
//!     }).await;
//!
//!     match result {
//!         Ok(loaded) => {
//!             // Start your framework with loaded.data
//!             App::start("app", |url, orders| init_with_data(loaded, orders), update, view);
//!         }
//!         Err(_) => {
